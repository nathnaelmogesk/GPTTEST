...
import { upsertMonthlyReview } from "@/app/actions";
...
export default async function MonthlyReviewPage({ searchParams }: PageProps) {
  ...
  const startDate = formatISO(monthStart, { representation: "date" });
  const endDate = formatISO(monthEnd, { representation: "date" });
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <p className="text-sm text-neutral-500">Monthly review</p>
          <h1 className="text-2xl font-semibold">{startDate}</h1>
        </div>
        <form action="/reviews/monthly" method="get" className="flex items-center gap-2">
          <Label>Month</Label>
          <Input type="date" name="month" defaultValue={startDate} />
      ...
      <Card>
        ...
        <CardContent>
          <form action={upsertMonthlyReview} className="grid grid-cols-1 gap-3 md:grid-cols-3">
